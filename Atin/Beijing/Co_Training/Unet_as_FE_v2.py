...
from Atin.Beijing.Co_Training.diag_train_v2 import Net as Diag_model, train as diag_train, Beijing_dataset as Beijing_diag_dataset, AddGaussian, auc_cal
from Atin.Beijing.Co_Training.seg_train_v2 import Seg_Model, seg_train, Beijing_Seg_Dataset, create_dark_mask, create_elastic_indices, train_transform, JaccardCriterion
...
class Combined_Model(nn.Module):
    ...
    def forward(self, x, seg_model):
        ...
        slice_id = 0
        for el in x:
            ind_output = self.diag(el)
            seg_output = seg_model(el)
            seg_output = seg_output.view(-1,32)
            ind_output = torch.cat([seg_output, ind_output],1)
...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(model_repo_dir,'unet_v2.pt')) and not Force_train:
        unet_model = torch.load(os.path.join(model_repo_dir,'unet_v2.pt'))
        ...
        torch.save(unet_model, os.path.join(model_repo_dir, 'unet_v2.pt'))
