...
#This has 64x1 vector in the last layer
...
def train(nb_seg_epoch, nb_epoch, model, seg_train_loader, diag_train_loader, criterion, optimizer, writer, alpha=.9):
    model.train()
    # for seg_epoch in range(1, nb_seg_epoch+1):
    #     seg_train(seg_epoch, model, seg_train_loader, criterion, optimizer)
    ...
    for epoch in range(1, nb_epoch + 1):
        # print(count)
        # new_count, _ = combined_train(epoch, model, seg_train_loader, diag_train_loader, criterion, optimizer, writer, count, alpha)
        # count = new_count
        predicted = []
        target = []
        for test_epoch in range(nb_of_test_epoch):
            current_predicted, current_target, _ = auc_cal(epoch, model, testloader, writer)
            predicted.append(current_predicted)
            target.append(current_target)
            # predicted = np.append(predicted,current_predicted,0)
            # target = np.append(target, current_target, 0)
        ...
        print("AUC fot Epoch {}: {:.4f}".format(epoch, test_auc[-1]))
...
def combined_train(epoch, model, seg_train_loader, diag_train_loader, criterion, optimizer, writer, count_writer, alpha=.9):
    model.train()
    train_loss = 0
    train_diag_loss = 0
    train_seg_loss = 0
...
def auc_cal_new(epoch, model, patient_id_H_test, patient_id_G_test, final_test, rep, writer, transform =None):
    model.eval()
    test_loss = 0
    correct = 0
    ...
    for patient_id in final_test:
        current_final_test = [patient_id]*rep
        diag_test_data = Beijing_Diag_Dataset(patient_id_H_test, patient_id_G_test, current_final_test, root_dir, (96, 288), transform)
        testloader = DataLoader(diag_test_data, batch_size=rep, num_workers=20, shuffle=True, collate_fn=my_collate, pin_memory=True)
        ...
        for _, (data, target) in enumerate(testloader):
            ...
            final_predicted.append(output / np.sum(output))
            final_target.append(target.data.cpu().numpy()[0])
    ...
    writer.add_pr_curve('PR curve', final_target, final_predicted[:, 1], epoch)
    ...
    print('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
        test_loss, correct, len(testloader.dataset),
        100. * correct / len(testloader.dataset)))
    return final_predicted, final_target, metrics.auc(fpr, tpr)


def train_new(nb_seg_epoch, nb_epoch, model, seg_train_loader, diag_train_loader, criterion, optimizer,
              patient_id_H_test, patient_id_G_test, final_test, rep, writer, transform,  alpha=.9):
    model.train()
    # for seg_epoch in range(1, nb_seg_epoch+1):
    #     seg_train(seg_epoch, model, seg_train_loader, criterion, optimizer)
    ...
    for epoch in range(1, nb_epoch + 1):
        ...
        for test_epoch in range(nb_of_test_epoch):
            # current_predicted, current_target, _ = auc_cal(epoch, model, testloader, writer)
            current_predicted, current_target, _ = auc_cal_new(epoch, model,
                                                               patient_id_H_test, patient_id_G_test, final_test, rep, writer, transform)
            predicted.append(current_predicted)
            target.append(current_target)
            # predicted = np.append(predicted,current_predicted,0)
            # target = np.append(target, current_target, 0)
...
def auc_cal(epoch, model, testloader, writer):
    ...
    for _, (data, target) in enumerate(testloader):
        ...
        #     final_predicted = np.concatenate(final_predicted, output / np.sum(output, axis=1).reshape(len(data),1),0)
        #     final_target = np.concatenate(final_target, target.data.cpu().numpy())
        # else:
        #     final_predicted = output / np.sum(output, axis=1).reshape(len(data),1)
        #     final_target = target.data.cpu().numpy()
...
if __name__ == "__main__":
    ...
    random.seed(231) #not good .83
    #random.seed(500) #Great result for balanced test
    #random.seed(8956) #moderaely ok
    #random.seed(567812) #bad
    ...
    ############
    # This code block should be there only for those models which can take all 6 images as input
    patient_id_G = set([x.split('_')[0] for x in os.listdir(root_dir + "/glaucoma/")])
    patient_id_H = set([x.split('_')[0] for x in os.listdir(root_dir + "/healthy/")])
    ###########
    ...
    #Balancing test data also
    # final_test = make_balance(patient_id_G_test, patient_id_H_test)
    ...
    testloader = DataLoader(diag_test_data, batch_size=20, num_workers=20, shuffle=True, collate_fn=my_collate, pin_memory=True)
    ...
    log_dir = os.path.join(root_log_dir, "co_training_with_any_slices_ex" + str(datetime.datetime.now()))
    os.makedirs(log_dir)
    writer = SummaryWriter(log_dir)

    transform =  T.Compose([Horizontalflip(), IntensityNonliearShift(), AddGaussian(ismulti=True),
                            AddGaussian(ismulti=False), RandomRotate(angle_range=(0, 5), prob=0, mode="constant"),
                            Translation(h_shift_range=(0, 4), w_shift_range=(0, 8), prob=0, mode="constant"), ToTensor()])
    # test_auc = train(nb_seg_epoch, nb_epoch, model, seg_train_loader, diag_train_dataloader, criterion, optimizer, writer, alpha=.8)
    test_auc = train_new(nb_seg_epoch, nb_epoch, model, seg_train_loader, diag_train_dataloader, criterion, optimizer,
                                patient_id_H_test, patient_id_G_test, final_test, 10, writer, transform, alpha=.9)
    ...
    # for epoch in range(1, nb_epoch + 1):
    #     # print(count)
    #     new_count, _ = train(epoch, model, seg_train_loader, diag_train_dataloader, criterion, optimizer, writer, count, alpha=.8)
    #     count = new_count
    #     predicted = []
    #     target = []
    #     for test_epoch in range(nb_of_test_epoch):
