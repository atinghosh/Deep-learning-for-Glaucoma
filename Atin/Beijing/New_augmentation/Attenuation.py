...
#ADDIng attenuation - a new augmentation
...
class Attenuation(object):
    ...
    def __call__(self, x):
        ...
        # cumsum_x = torch.cumsum(x,1)
        # a = cumsum_x.numpy()
        # np.min(a[np.nonzero(a)])
        # torch.min(cumsum_x[torch.nonzero(cumsum_x)])
        # lambda_min = math.log(.8)/(torch.min(cumsum_x)+.01)
        # lambda_min = math.log(.8) / (torch.min(cumsum_x) + .01)
        # lambda_max = math.log(1.2) / (torch.max(cumsum_x)+.01)
        # Lambda = np.random.uniform(lambda_min, lambda_max,1)

        x_transform = torch.exp(-Lambda[0] * torch.cumsum(x,1)) * x
        # x_transform = x_transform/torch.max(x_transform)
        # plt.imshow(x.numpy()[0])
        # plt.imshow(x_transform.numpy()[0])

        return x_transform


class Attenuation_grid(object):
    ...
    def __call__(self, x):
        # Lambda = np.random.uniform(self.range[0], self.range[1],1)
        Lambda = np.random.uniform(self.range[0], self.range[1], size=(1,1,x.size(2)))
        Lambda = torch.from_numpy(Lambda).type(torch.FloatTensor)
        x_transform = torch.exp(-Lambda * torch.cumsum(x,1)) * x
        # plt.imshow(x.numpy()[0])
        # plt.imshow(x_transform.numpy()[0])
...
if __name__ == "__main__":
    ...
    transform_pipeline_train = tr.Compose([AddGaussian(),tr.ToTensor(), tr.AddChannel(axis=0), tr.TypeCast('float'),
                         # tr.RangeNormalize(0,1),
                         # Attenuation((-.001,.2)),
                         # Attenuation_grid((-.001, .2)),
                         tr.RandomBrightness(-.2, .2),
                         tr.RandomGamma(.8,1.2),
                         tr.RandomFlip(),
                         tr.RandomAffine(rotation_range=5, translation_range=0.2, zoom_range=(0.9,1.1))])

    transform_pipeline_test = tr.Compose([tr.ToTensor(), tr.AddChannel(axis=0), tr.TypeCast('float')
                                           # tr.RangeNormalize(0, 1)
                                          ])

    transformed_images = Beijing_dataset(root_dir, patient_id_G_train, patient_id_H_train, transform= transform_pipeline_train)
