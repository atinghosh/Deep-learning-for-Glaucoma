...
#Works great so far
#Only slice 1 and slice 6 are taken, there is a parameter corresponding to every slice
...
class Net_final(nn.Module):
    ...
    def __init__(self):
        ...
        self.alpha = nn.Parameter(torch.randn(5, requires_grad = True).cuda())
    ...
    def forward(self,y):
        ...
        for x in y:
            ...
            if slice_id !=0:
                al = torch.log(1+torch.exp(self.alpha[slice_id-1]))
                slice_parameter.append(al.data.cpu().numpy())
...
if __name__ == "__main__":
    ...
    transform_pipeline_train = tr.Compose([AddGaussian(), AddGaussian(ismulti=False),tr.ToTensor(), tr.AddChannel(axis=0), tr.TypeCast('float'),
                                           # Attenuation((-.001, .1)),
